...
#define ARM_USART_ABORT_SEND                (0x18UL << ARM_USART_CONTROL_Pos)   /* Abort ARM_USART_Send */
#define ARM_USART_ABORT_RECEIVE             (0x19UL << ARM_USART_CONTROL_Pos)   /* Abort ARM_USART_Receive */
#define ARM_USART_ABORT_TRANSFER            (0x1AUL << ARM_USART_CONTROL_Pos)   /* Abort ARM_USART_Transfer */
 ...
 * - data_out: Pointer to buffer with data to send to USART transmitter
 * - data_in: Pointer to buffer for data to receive from USART receiver
...
typedef struct _ARM_DRIVER_USART
{
    ...
    ARM_USART_CAPABILITIES (*GetCapabilities) (void);                              /* Get driver capabilities. */
    int32_t                (*Initialize)      (ARM_USART_SignalEvent_t cb_event);  /* Initialize USART Interface. */
    int32_t                (*Uninitialize)    (void);                              /* De-initialize USART Interface. */
    int32_t                (*PowerControl)    (ARM_POWER_STATE state);             /* Control USART Interface Power. */
    int32_t                (*Send)            (const void *data, uint32_t num);    /* Start sending data to USART transmitter. */
    int32_t                (*Receive)         (      void *data, uint32_t num);    /* Start receiving data from USART receiver. */
    int32_t                (*Transfer)        (const void *data_out,
                                                     void *data_in,
                                               uint32_t    num);                   /* Start sending/receiving data to/from USART. */
    uint32_t               (*GetTxCount)      (void);                              /* Get transmitted data count. */
    uint32_t               (*GetRxCount)      (void);                              /* Get received data count. */
    int32_t                (*Control)         (uint32_t control, uint32_t arg);    /* Control USART Interface. */
    ARM_USART_STATUS       (*GetStatus)       (void);                              /* Get USART status. */
    int32_t                (*SetModemControl) (ARM_USART_MODEM_CONTROL control);   /* Set USART Modem Control line state. */
    ARM_USART_MODEM_STATUS (*GetModemStatus)  (void);                              /* Get USART Modem Status lines state. */
