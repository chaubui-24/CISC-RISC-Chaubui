...
/* * Macro for Access Code Call.
 * On ARM/Thumb, BLX instruction used by the compiler for calling a function
 * pointed to by the pointer requires that LSB bit of the address is set to one
 * if the called fcn is coded in Thumb.
 */
#define MEM_43_INFLS_AC_CALL(ptr2fcn, ptr2fcnType) \
    ((ptr2fcnType)(((uint32_t)(ptr2fcn)) | MEM_43_INFLS_ARM_FAR_CALL2THUMB_CODE_BIT0_U32))
...
/* Program Address and Data (8-byte / phrase) into Flash Memory */
uint8_t Program_LongWord_8B(uint32_t Addr, uint8_t *Data)
{
    ...
    /* Program 8 bytes (phrase) in a program flash block */
...
{
    /* Clear flags or handle Flash command complete/collision interrupts here */
}
